...
Vector3f Quat4f::xyz() const { return Vector3f(m_elements[1], m_elements[2], m_elements[3]); }
...
Vector3f Quat4f::getAxisAngle(float *radiansOut) {
  ...
  *radiansOut = theta;
  return Vector3f(x() * reciprocalVectorNorm, y() * reciprocalVectorNorm,
...
Quat4f Quat4f::slerp(const Quat4f &a, const Quat4f &b, float t, bool allowFlip) {
  ...
  return Quat4f(c1 * a[0] + c2 * b[0], c1 * a[1] + c2 * b[1], c1 * a[2] + c2 * b[2],
                c1 * a[3] + c2 * b[3]);
}

// static
Quat4f Quat4f::squad(const Quat4f &a, const Quat4f &tanA, const Quat4f &tanB, const Quat4f &b,
                     float t) {
...
// static
Quat4f Quat4f::cubicInterpolate(const Quat4f &q0, const Quat4f &q1, const Quat4f &q2,
...
Quat4f Quat4f::randomRotation(float u0, float u1, float u2) {
  ...
  return Quat4f(cos(w), sin(w) * cos(theta) * r, sin(w) * sin(theta) * r, sin(w) * z);
}
