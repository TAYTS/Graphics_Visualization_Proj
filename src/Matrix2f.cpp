...
Matrix2f::Matrix2f(const Matrix2f &rm) { memcpy(m_elements, rm.m_elements, 2 * sizeof(float)); }
...
const float &Matrix2f::operator()(int i, int j) const { return m_elements[j * 2 + i]; }

float &Matrix2f::operator()(int i, int j) { return m_elements[j * 2 + i]; }

Vector2f Matrix2f::getRow(int i) const { return Vector2f(m_elements[i], m_elements[i + 2]); }
...
Matrix2f Matrix2f::inverse(bool *pbIsSingular, float epsilon) {
  ...
  if (isSingular) {
    ...
    return Matrix2f(m_elements[3] * reciprocalDeterminant, -m_elements[2] * reciprocalDeterminant,
...
Matrix2f operator*(const Matrix2f &m, float f) { return f * m; }
